...
std::tuple<at::Tensor, at::Tensor> ROIPool_forward_cuda(const at::Tensor& input,
  ...
  at::Tensor argmax = at::empty({num_rois, channels, pooled_height, pooled_width}, input.options()).to(at::kInt).zero_();
...
at::Tensor ROIPool_backward_cuda(const at::Tensor& grad,
  ...
  auto num_rois = rois.size(0);
  at::Tensor grad_input = at::empty({batch_size, channels, height, width}, grad.options()).zero_();
