...
TODO: these are only a subset of rle kernels. We may want to write
cuda kernels for all functions in maskApi*/

typedef unsigned int uint;
typedef unsigned long siz;
typedef unsigned char byte;
const int BUFFER_SIZE=6144;
const int CNTS_SIZE=1024;

__global__ void crop_and_scale_cuda_kernel(double * dense_poly_data, int * per_anchor_poly_idx, int* poly_rel_idx, int poly_count, int anchor_count, float * anchor_data, int mask_size){
    int tid = threadIdx.x;
    ...
    float w = anchor_data[4*anchor_idx+2] - anchor_data[4*anchor_idx];
  	float h = anchor_data[4*anchor_idx+3] - anchor_data[4*anchor_idx+1];
  	w=fmaxf(w,1.0f);
  	h=fmaxf(h,1.0f);
    ...
    for (int j=tid;j<len;j+=block_jump){
        if (j%2==0) poly_data_buf[j] = ratio_w*((float)poly_data_buf[j]- anchor_data[4*anchor_idx]);
        if (j%2==1) poly_data_buf[j] = ratio_h*((float)poly_data_buf[j]- anchor_data[4*anchor_idx+1]);
    }
}

__global__ void merge_masks_cuda_kernel(byte * masks_in, float * masks_out, int mask_size, int* per_anchor_poly_idx, int anchor_count){
...
__global__ void decode_rle_cuda_kernel(const int * num_of_cnts, uint * cnts, long h, long w, byte * mask)
{
    ...
    for(int j=tid; j<h*w; j+=block_jump){
       // int transposed_pixel = (j%h)*w+j/h; //make the mask row major, rle encodes col major
        int bin=0;
        int k;
        for(k=0;k<=m;k++){
...
__global__ void rle_fr_poly_cuda_kernel(const double * dense_coordinates, int * poly_rel_idx, long h, long w, uint * cnts, int* x_in, int * y_in, int* u_in, int* v_in, uint* a_in, uint* b_in, int * num_of_cnts)
{
    ...
    for(long j=tid; j<BUFFER_SIZE; j+= block_jump) {shbuf1[j]=0;shbuf2[j]=0;}
    for(long j=tid; j<=k; j+=block_jump) x[j] = j<k ? ((int) (scale*xy[2*j+0] + 0.5)) : ((int) (scale*xy[0] + 0.5));
    for(long j=tid; j<=k; j+=block_jump) y[j] = j<k ? ((int) (scale*xy[2*j+1] + 0.5)) : ((int) (scale*xy[1] + 0.5));
    __syncthreads();
     ...
     for(int j=tid; j<k; j+=block_jump){
        ...
        shbuf1[j+1]=dist;shbuf2[j+1]=dist;
    }
      __syncthreads();
     ...
     for(int j=tid; j<k; j+=block_jump){
        int xs=x[j], xe=x[j+1], ys=y[j], ye=y[j+1], dx, dy, t, d, dist;
        int flip; double s; dx=__sad(xe,xs,0); dy=__sad(ys,ye,0);
        flip = (dx>=dy && xs>xe) || (dx<dy && ys>ye);
        if(flip) { t=xs; xs=xe; xe=t; t=ys; ys=ye; ye=t; }
        s = dx>=dy ? (double)(ye-ys)/dx : (double)(xe-xs)/dy;
        m=switch_buf==0 ? shbuf2[j]:shbuf1[j];
       if(dx>=dy) for( d=0; d<=dx; d++ ) {
          //using __dmul_rn forces ftz=False with round-to-nearest-even mode;
          //regular s*t is computed with ftz=True with compiler optimizations,
          //which doesn't match CPU result
          t=flip?dx-d:d; u[m]=t+xs; v[m]=(int)(ys+__dmul_rn(s,t)+.5); m++;
        } else for( d=0; d<=dy; d++ ) {
          t=flip?dy-d:d; v[m]=t+ys; u[m]=(int)(xs+__dmul_rn(s,t)+.5); m++;
        }
    }
    __syncthreads();
...
at::Tensor generate_mask_targets_cuda( at::Tensor dense_vector, std::vector<std::vector<at::Tensor>> polygons, at::Tensor anchors, int mask_size){
    ...
    *(poly_rel_idx.data<int>()+poly_count) = start_idx;
    ...
    at::Tensor d_x_t = at::empty({BUFFER_SIZE*num_of_poly}, at::CUDA(at::kInt));
    at::Tensor d_y_t = at::empty({BUFFER_SIZE*num_of_poly}, at::CUDA(at::kInt));
    at::Tensor d_u_t = at::empty({BUFFER_SIZE*num_of_poly}, at::CUDA(at::kInt));
    at::Tensor d_v_t = at::empty({BUFFER_SIZE*num_of_poly}, at::CUDA(at::kInt));
    at::Tensor d_a_t = at::empty({BUFFER_SIZE*num_of_poly}, at::CUDA(at::kInt));//used with uint later
    at::Tensor d_b_t = at::empty({BUFFER_SIZE*num_of_poly}, at::CUDA(at::kInt)); //used with uint later
    at::Tensor d_mask_t = at::empty({M*M*num_of_poly}, at::CUDA(at::kByte));
    auto result =  at::empty({num_of_anchors,M,M}, torch::CUDA(at::kFloat));
    at::Tensor d_num_of_counts_t = at::empty({num_of_poly}, at::CUDA(at::kInt));
    at::Tensor d_cnts_t = at::empty({CNTS_SIZE*num_of_poly}, at::CUDA(at::kInt));
    ...
    rle_fr_poly_cuda_kernel<<<num_of_poly,256,0,stream.stream()>>>(d_dense_vector.data<double>(),
                                                                 d_poly_rel_idx.data<int>(),
                                                                 M, M,
                                                                 (unsigned int*) d_cnts_t.data<int>(),
                                                                 d_x_t.data<int>(),
                                                                 d_y_t.data<int>(),
                                                                 d_u_t.data<int>(),
                                                                 d_v_t.data<int>(),
                                                                 (unsigned int*) d_a_t.data<int>(),
                                                                 (unsigned int*) d_b_t.data<int>(),
                                                                 d_num_of_counts_t.data<int>());
    decode_rle_cuda_kernel<<<num_of_poly,256,0,stream.stream()>>>(d_num_of_counts_t.data<int>(),
                                                                 (unsigned int*) d_cnts_t.data<int>(),
                                                                 M, M,
                                                                 d_mask_t.data<byte>());
    merge_masks_cuda_kernel<<<num_of_anchors,256,0,stream.stream()>>>(d_mask_t.data<byte>(), result.data<float>(), M, d_per_anchor_poly_idx.data<int>(), num_of_anchors);
