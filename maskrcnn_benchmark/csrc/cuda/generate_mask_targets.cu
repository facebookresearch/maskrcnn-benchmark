...
at::Tensor generate_mask_targets_cuda(at::Tensor dense_vector, const std::vector<std::vector<at::Tensor>> polygons, 
    ...
    *(poly_rel_idx.data<int>() + poly_count) = start_idx;
    ...
    at::Tensor d_a_t = at::empty({BUFFER_SIZE * num_of_poly}, at::CUDA(at::kInt));//used with uint* pointer
    at::Tensor d_b_t = at::empty({BUFFER_SIZE * num_of_poly}, at::CUDA(at::kInt)); //used with uint* pointer
