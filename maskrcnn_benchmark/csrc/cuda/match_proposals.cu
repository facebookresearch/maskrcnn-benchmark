...
__launch_bounds__(256) static __global__
    void max_along_gt_idx(float* match, unsigned char * pred_forgiven, long* max_gt_idx, long long gt,long long preds, bool include_low_quality, float low_th, float high_th) {
...
__launch_bounds__(256) static __global__
    void forgive_preds(float * match_quality_data, float * d_best_pred_per_gt, unsigned char * d_pred_forgiven, long gt, long preds){
...
at::Tensor match_proposals_cuda(at::Tensor match_quality_matrix, bool allow_low_quality_matches, float low_th, float high_th){
    ...
    using namespace at;
    //predictions are reduced by chunks of 2048 elements per CTA
    int num_chunks = (preds+2047)/2048;
    auto result = at::ones({preds},torch::CUDA(at::kLong));
    at::Tensor best_pred_per_gt = at::zeros({gt}, at::CUDA(at::kFloat));
    at::Tensor pred_forgiven = at::zeros({preds}, at::CUDA(at::kByte));
    at::Tensor intergt = at::zeros({gt*num_chunks}, at::CUDA(at::kFloat));
    ...
    if (allow_low_quality_matches) max_along_preds<<<grid,block,0,stream.stream()>>>(match_quality_matrix.data<float>(), intergt.data<float>(), gt, preds);
    //final reduction to find best iou per gt
    int numThreads=256;
    int numBlocks=(gt+255)/256;

    if (allow_low_quality_matches) max_along_preds_reduced<<<numBlocks,numThreads,0,stream.stream()>>>(intergt.data<float>(), best_pred_per_gt.data<float>(), gt, num_chunks);
    numBlocks=(preds+255)/256;
    //if low_quality_matches are allowed, mark some predictions to keep their best matching gt even though iou<threshold
    if (allow_low_quality_matches) forgive_preds<<<numBlocks,numThreads,0,stream.stream()>>>(match_quality_matrix.data<float>(), best_pred_per_gt.data<float>(), pred_forgiven.data<unsigned char>(), gt, preds);
    //compute resulting tensor of indices
    max_along_gt_idx<<<numBlocks,numThreads,0,stream.stream()>>>(match_quality_matrix.data<float>(), pred_forgiven.data<unsigned char>(), result.data<long>(), gt, preds,allow_low_quality_matches, low_th, high_th);
