...
__global__ void box_encode_kernel(float * targets_dx, float * targets_dy, float * targets_dw, float * targets_dh,
        float4 * boxes, float4 * anchors, float wx, float wy, float ww, float wh, size_t gt, size_t idxJump) {
...
std::vector<at::Tensor> box_encode_cuda( at::Tensor boxes, at::Tensor anchors, float wx, float wy, float ww, float wh){
    ...
    box_encode_kernel<<<gridDim,blockDim,0,stream.stream()>>>(targets_dx.data<float>(), targets_dy.data<float>(), targets_dw.data<float>(), targets_dh.data<float>(), (float4*) boxes.data<float>(), (float4*) anchors.data<float>(), wx, wy, ww, wh, size, idxJump);
