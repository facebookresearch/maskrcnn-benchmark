...
This folder contains a simple video demo that illustrates how you can use `maskrcnn_benchmark` for inference.

You can start it by running it from this folder, using one of the following commands:
```bash
python --video-file "<path_to_video>video.mp4" --config-file "../configs/caffe2/e2e_mask_rcnn_R_101_FPN_1x_caffe2.yaml"
--confidence-threshold 0.7 --min-image-size 800 MODEL.DEVICE cuda MODEL.MASK_ON True EXTRACT_MASK.ENABLE True EXTRACT_MASK.TRANSPARENT True EXTRACT_MASK.RESIZE True EXTRACT_MASK.DSIZE 800
